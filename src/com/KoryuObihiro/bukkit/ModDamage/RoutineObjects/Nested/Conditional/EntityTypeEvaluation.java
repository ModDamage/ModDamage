...
public class EntityTypeEvaluation extends EntityConditionalStatement
{
	...
	{
		ConditionalRoutine.registerConditionalStatement(Pattern.compile("(!?)(.*)\\.type\\.(.*)", Pattern.CASE_INSENSITIVE), new StatementBuilder());
	}
	...
	{	
		...
		{
			Collection<ModDamageElement> elements = AliasManager.matchTypeAlias(matcher.group(3));
			if(!elements.isEmpty())
